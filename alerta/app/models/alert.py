...
class History(object):
    ...
    def __init__(self, id, event, **kwargs):
        ...
        self.change_type = kwargs.get('change_type', None) or kwargs.get('type', None) or ""
...
class Alert(object):
    ...
    def from_document(cls, doc):
        return Alert(
            ...
            history=[History(
                ...
                change_type=h.get('type', None),
                text=h.get('text', None),
                update_time=h.get('updateTime', None)
            ) for h in doc.get('history', list())]
        )
    ...
    def deduplicate(self):
        return Alert.from_db(db.dedup_alert(self))
    ...
    def set_status(self, status, text=''):
        return db.set_status(self.id, status, text)
    ...
    # search alerts
    @staticmethod
    def find_all(query=None, sort=None, page=1, page_size=0):
        return [Alert.from_db(alert) for alert in db.get_alerts(query, sort, page, page_size)]

    # list alert history
    @staticmethod
    def get_history(query=None, fields=None, page=1, page_size=0):
        return db.get_history(query, fields, page, page_size)
